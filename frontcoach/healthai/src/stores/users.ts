...
export const useUserStore = defineStore('users', () => {
  ...
  const createUser = async (userData: Omit<User, 'User_ID' | 'created_at' | 'updated_at'> & { User_password: string }) => {
    loading.value = true
    error.value = null
    ...
    try {
      ...
      error.value = err.response?.data?.detail || 'Erreur lors de la creation'
