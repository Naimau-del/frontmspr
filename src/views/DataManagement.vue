  ...
  return [
    ...
    },
    {
      ...
      percentage: products.length > 0
        ? Math.round((products.filter(p => p.product_kcal && p.product_protein).length / products.length) * 100)
        : 0,
      details: `${products.filter(p => p.product_kcal && p.product_protein).length} sur ${products.length} produits`
    },
    {
